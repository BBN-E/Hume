...
Python API for Accessing SerifXML Files.
    ...
    >>> serif_doc = serifxml.send_serifxml_document(document_text)
    >>> print serif_doc
    Document:
      docid = u'anonymous'
      language = u'English'
...
class SerifSynNodeTheory(SerifSequenceTheory):
    ...
    def __repr__(self):
        ...
        # Note: __repr__ is required to return a string (not unicode)
...
class Fact(SerifTheory):
    ...
    text_span_fact_arguments = _ChildTheoryElementList('StringFactArgument')
...
<SerifXMLRequest>
  <ProcessDocument end_stage="%(end_stage)s" output_format="serifxml" input_type="%(input_type)s">
    %(document)s
  </ProcessDocument>
</SerifXMLRequest>
'''
...
def send_serifxml_pd_request(document, hostname=HOSTNAME, port=PORT,
                             end_stage='output', input_type='auto', verbose=False, timeout=0, num_tries=1):
    """
...
def send_serifxml_document(content, docid='anonymous', language='English',
    ...
    return send_serifxml_pd_request(xml_request, end_stage=end_stage, input_type=input_type,
                                    verbose=verbose, hostname=hostname, port=port, timeout=timeout, num_tries=num_tries)
...
if __name__ == '__main__':
    ...
    if args[0] == 'process_batch_file':
        ...
        files = [x.strip() for x in open(opts.input_batch_file, 'rb').readlines()]
        for index in range(len(files)):
            input_file = files[index]
            filename = os.path.basename(input_file)
            print("Processing file (%d of %d): %s" % (index, len(files), filename))
