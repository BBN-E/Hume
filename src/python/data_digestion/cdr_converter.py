import os,sys,json,shutil,datetime,random,io
...
def main(input_cdr_list,output_folder,breaking_point=30000):
    ...
    for doc in scanner:
        ...
        if is_news(doc):
            make_sgm_file(extracted_text,doc_uuid,output_folder,os.path.join(output_folder,'cdrs',doc['document_id']+".json"),source_uri,"news_{}".format(doc_uuid),"ENG_NW_WM",creation_date_str,author,booking_arr,sgm_path_list,breaking_point)
        else:
            make_sgm_file(extracted_text,doc_uuid,output_folder,os.path.join(output_folder,'cdrs',doc['document_id']+".json"),source_uri,"analytic_{}".format(doc_uuid),"ENG_NW_WM",creation_date_str,author,booking_arr,sgm_path_list,breaking_point)
