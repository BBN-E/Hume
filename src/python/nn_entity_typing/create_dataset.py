...
def create_dataset(corpus_path,label2id,word2id,feature2id):
    ...
    with open(corpus_path) as f:
        for line in f:
            ...
            features_code = [feature2id[feature] if feature in feature2id else feature2id["unknown_feature_case"] for feature in features]
            storage += words_code
...
def main():
    ...
    re_serialize_input_dict = False
    if "unknown_word_case" not in word2id:
        word2id["unknown_word_case"] = max(word2id.values())+1
        re_serialize_input_dict = True
    feature2id = dicts["feature2id"]
    if "unknown_feature_case" not in feature2id:
        feature2id["unknown_feature_case"] = max(feature2id.values())+1
        re_serialize_input_dict = True
    ...
    if re_serialize_input_dict:
        joblib.dump(dicts,sys.argv[1])
