...
class Model:
    def __init__(self,type, encoder, hier, feature,target_dim):
	...
	# Bonan: 56 is the number of classes
        ...
        if hier:
            ...
            _d = "bbn-corpus" if type == "figer" else "OntoNotes"
    ...
    def save(self,file_path,dict_path):
        print "Saving tenserflow session object..."
        saver = tf.train.Saver()
        saver.save(self.session,file_path)
        # Save the model init params in json so that model can be reconstructed with these params
        # We do this since serializing the Model object itself is having some issues (that's why 
        # we are saving the session separately). This could be a temporary hack.
        model_init_params={"type":self.type, "encoder":self.encoder, "hier":self.hier, "feature":self.feature,\
                          "target_dim":self.target_dim}
        output_dir=file_path[0:file_path.rfind(os.path.sep)]
        try:
            os.makedirs(output_dir)
        except:
            pass
        print "Saving model init params..."
        with open(os.path.join(output_dir,'model_init_params.json'),'w') as fp:
            json.dump(model_init_params,fp)
        shutil.copy(dict_path,output_dir)
       
    @staticmethod
    def load(file_path):
        print "Recreating model object..."
        model = None
        output_dir=file_path[0:file_path.rfind(os.path.sep)]
        with open(os.path.join(output_dir,'model_init_params.json')) as fp:
            params = json.load(fp)
            model = Model(params['type'],params['encoder'],params['hier'],params['feature'],params['target_dim'])
        if not model:
            raise ValueError("Cannot restore model from init params!")
        saver = tf.train.Saver()
        print ("Restoring session...")
        saver.restore(model.session,file_path)
        return model
