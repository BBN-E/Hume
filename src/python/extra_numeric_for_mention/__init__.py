...
should_not_cross_word = {'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September',
                         'October', 'November', 'December', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug',
                         'Sep', 'Oct', 'Nov', 'Dec', 'to', "Week", ","}
...
def my_tokenizer(sent_str):
    ...
    for token in tokens:
        if len(token) > 0:
            resolved_tokens.append(token.replace(",", ""))
...
def left_judger(token,status):
    ...
    if token.isdigit():
        ...
        return True,{'continue':general_should_continue(token,status,text_window,should_not_cross_word)}
    elif token in number_str_to_multiplier:
        return True,{'continue':general_should_continue(token,status,text_window,should_not_cross_word)}
    if len(token) < 1:
...
def extract_numeric_for_mention(sent_str,mention_str):
    ...
    sentence_tokens = my_tokenizer(sent_str)
    if len(mention_tokens) < 1 or len(sentence_tokens) < 1:
        return dict()
    start_mention_token_idx = -1
    end_mention_token_idx = -1
    for idx,token in reversed(list(enumerate(sentence_tokens))):
        if token == mention_tokens[0] and idx + len(mention_tokens) - 1 < len(sentence_tokens):
            if sentence_tokens[idx+len(mention_tokens)-1] == mention_tokens[-1]:
    ...
    for idx in range(start_mention_token_idx-1,-1,-1):
        ...
        if token in should_not_cross_word:
            continue
        # if idx-1 >= 0 and sentence_tokens[idx-1] in should_not_cross_word:
        #     continue
        # if idx-2 >= 0 and sentence_tokens[idx-2] in should_not_cross_word:
        #     continue
        # if idx-3 >= 0 and sentence_tokens[idx-3] in should_not_cross_word:
        #     continue
        # if idx +1 < len(sentence_tokens) and sentence_tokens[idx+1] in should_not_cross_word:
        #     continue
        # if idx +2 < len(sentence_tokens) and sentence_tokens[idx+2] in should_not_cross_word:
        #     continue
        # if idx +3 < len(sentence_tokens) and sentence_tokens[idx+3] in should_not_cross_word:
        #     continue
    ...
    # In the mention
    status = dict()
    for idx in range(start_mention_token_idx,end_mention_token_idx+1):
        status['current_idx'] = idx
        status['init_idx'] = idx
        token = sentence_tokens[idx]
        if token in should_not_cross_word:
    ...
    # Going right
    status = dict()
    for idx in range(end_mention_token_idx+1,len(sentence_tokens),1):
        status['current_idx'] = idx
    ...
    modifier_token = [sentence_tokens[idx].replace(",","") for idx in modifiers_idx_list]
    ...
    for idx,token in enumerate(sentence_tokens):
        if token in {"per","each"} and idx + 1 < len(sentence_tokens) and sentence_tokens[idx+1] in available_word_after_per:
            possible_val = None
            try:
                start_idx = modifier_token.index("each")
                for idx_2 in range(start_idx,-1,-1):
                    try:
                        possible_val = float(modifier_token[idx_2])
                    except:
                        pass
            except ValueError:
                pass
            ...
            if possible_val is not None:
                sol['time_interval'] = "{} {}".format(token,sentence_tokens[idx+1])
    ...
    if len(modifier_token_set.intersection(number_str_to_multiplier.keys())) > 0:
        possible_multiplier = 1
        possible_num = None
        for token in modifier_token:
            if token in number_str_to_multiplier:
            ...
            try:
                ...
                pass
        if possible_num is not None:
            sol['val'] = possible_num * possible_multiplier
            return sol
    possible_multiplier = 1
    possible_nums = set()
    for token in modifier_token:
...
if __name__ == "__main__":
    entries = [
        ...
        # {"sent_str":"over 80000 people","mention":"over 80000 people","desired_modifiers":["over","80000"]},
        #####
        ...
        # {"sent_str":"30 April 296,748 Refugees in South Sudan and 1.76 million IDPs including 202,154 in UNMISS Protection of Civilians sites as of 22 February 2018 SOUTH SUDANESE REFUGEES AS OF 30 APRIL 2018","mention":"30 April 296,748 Refugees in South Sudan and 1.76 million IDPs including 202,154 in UNMISS Protection of Civilians sites as of 22 February 2018 SOUTH SUDANESE REFUGEES AS OF 30 APRIL 2018","desired_modifiers":[]},
        # {"sent_str":"1.76 million IDPs","mention":"1.76 million IDPs","desired_modifiers":[]},
    #
