...
Instance = collections.namedtuple('Instance',['left_mention','relation','right_mention','score','sentence'])
...
def single_document_handler(serif_path):
    ...
    for serif_eerm in serif_doc.event_event_relation_mention_set or []:
        ...
        if serif_em_arg1 is not None and serif_em_arg2 is not None:
            ...
            entries.append(Instance(left_em_text,relation_type,right_em_text,confidence,sentence_text))
...
def main(serif_list,prefix):
    ...
    for i in sorted(eer_list,key=lambda x:(x.relation,-x.score)):
        print("{}\t{}\t{}\t{}\t{}\t{}".format(prefix,i.left_mention,i.relation,i.right_mention,i.score,i.sentence))
