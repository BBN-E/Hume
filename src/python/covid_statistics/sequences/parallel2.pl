...
### Please Change this
my $JOB_NAME = "covid_statistics_collection_2";
my $NUMBER_OF_BATCHES = 2000;
my $input_aylien_list = "/d4m/ears/expts/48076.aylien.full.nn_eer_only.041221.v1/expts/mtdp/nlplingo_eer/serif.list";
(my $processing_dir, undef) = Utils::make_output_dir("$exp_root/expts/$JOB_NAME", "$JOB_NAME/mkdir_output", []);
### End please Change this

{
    my $stage_name = "covid_statistics_collection_2";
        ...
        for (my $n = 0; $n < $NUMBER_OF_BATCHES; $n++) {
            ...
            my $jobid1 =
                runjobs(
                    ...
                    [ "env PYTHONPATH=$textopen_root/src/python python3 $hume_repo_root/src/python/covid_statistics/eer_analysis_2.py both_end_typed $batch_file $output_serifxml_dir/eer.dump" ]
                );
        }
    }
