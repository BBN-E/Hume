...
class LearnItObversationFeatureExtractorRaw(FeatureExtractor):

    def __init__(self,learnit_jar_path,learnit_target,learnit_param,serif_list,input_bert_list,cap,output_path):
        super().__init__()
        self.learnit_jar_path = learnit_jar_path
        self.learnit_target = learnit_target
        self.learnit_param = learnit_param
        self.serif_list = serif_list
        self.input_bert_list = input_bert_list
        self.output_path = output_path
        self.cap = int(cap)
    def extract_features(self):
        sub_p = subprocess.run(shlex.split("java -cp {} {} {} {} {} {} {} {}".format(
            self.learnit_jar_path,
            "com.bbn.akbc.neolearnit.util.SampleLearnItObversationInstancesFromSerifXML",
            self.learnit_param,
            self.learnit_target,
            self.serif_list,
            "LearnitPattern",
            self.cap,
            os.path.join(self.output_path,"selected_ins.json")
        )))
        with open(os.path.join(self.output_path,"selected_ins.json")) as fp:
