...
public class AddEventMentionFromIDA {
    ...
    public static Optional<SynNode> findAnchorNode(DocTheory docTheory, SentenceTheory sentenceTheory, EventMentionInfoLDC eventMentionInfo) {
        ...
        for(int tid = sentenceTheory.tokenSequence().size()-1; tid>=0; tid--) {
            ...
            System.out.println(Integer.toString(token_start));
            System.out.println(Integer.toString(token_end));
    ...
    public static DocTheory addEventMentions(final DocTheory input) {
        ...
        for(EventMentionInfoLDC eventMentionInfo : ldc_docid2eventmentions.get(input.docid().asString())) {
            ...
            if(!foundSynNode) {
                ...
                throw new IllegalArgumentException("event mention not found");
            }
        }
    ...
    public static Multimap<String, EventMentionInfoLDC> load_ldc_event_mentions(String fileLdcAnnotation, Multimap<String, EventMentionInfoLDC> docid2eventmentions) throws IOException {
        ...
        for (String line : lines) {
            ...
            try {
                ...
                String trigger_text = items[3];
                long char_start = Integer.parseInt(items[1]);
                long char_end = Integer.parseInt(items[2]);
                Optional<Pair<Long, Long>> absent = Optional.absent();

                num_lines += 1;
                if (DEBUG) {
                    System.out.println("docid\t" + docid + "\t" + char_start + "\t" + char_end);
                }

                Multimap<String, Map<String, String>> roleOffset = HashMultimap.create();
                EventMentionInfoLDC eventMentionInfo = new EventMentionInfoLDC("None", docid, absent, new Pair<Long, Long>(char_start, char_end), roleOffset, 1.0f, Integer.toString(eventCount));
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (final DocTheory dt : SerifIOUtils.docTheoriesFromFiles(serifFilesToProcess, serifXMLLoader)) {
            ...
            String strOutputSerifXml = strOutputDir + "/" + docTheoryWithNewEvents.docid().asString() + ".xml";
