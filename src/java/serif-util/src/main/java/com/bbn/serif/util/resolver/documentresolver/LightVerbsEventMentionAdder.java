...
// +2
public final class LightVerbsEventMentionAdder implements DocumentResolver, Resolver {

  ImmutableSet<String> lightVerbs;

  public LightVerbsEventMentionAdder(String lightVerbsFile) throws IOException {
    lightVerbs =
        ImmutableSet
  ...
  public final DocTheory resolve(final DocTheory docTheory) {
    ...
    for (int i = 0; i < docTheory.numSentences(); ++i) {
      ...
      ImmutableList<EventMention> modifiedEventMentions =
          EventConsolidator
              .addLightVerbs(lightVerbs, listBuilder.build(), sentenceTheory, docid, sentenceNPs);
