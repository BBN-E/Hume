...
public final class MaxSimPredicateFeature extends PairFeature {
  private static final Logger log = LoggerFactory.getLogger(MaxSimPredicateFeature.class);

  private MaxSimPredicateFeature(final String featureName,
  ...
  public static class Builder {
    ...
    public MaxSimPredicateFeature build() {
      ...
      if(indices.size() > 0) {
        ...
        return new MaxSimPredicateFeature(featureName, features.build(), featureIndices, minIndex,
            maxIndex);
      } else {
        return new MaxSimPredicateFeature(featureName, features.build(), featureIndices, -1, -1);
      }
    }
