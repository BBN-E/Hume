 ...
 */
public final class DirectPropRole extends PairFeature {
      ...
      for (final SymbolPair idPair : idPairs) {
        for(final Symbol item : idPair.getMembers()) {
          ...
          if(!seenIds.contains(item)) {
            ...
            com.bbn.necd.event.features.single.DirectPropRole.insertFeatures(featureName, featuresCacheBuilder, eg, item);
