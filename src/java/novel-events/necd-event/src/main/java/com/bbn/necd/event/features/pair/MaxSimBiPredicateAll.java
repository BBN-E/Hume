 ...
 */
public final class MaxSimBiPredicateAll extends PairFeature {
  private MaxSimBiPredicateAll(final String featureName,
  ...
  public static class Builder {
    ...
    public Builder extractFeatures(final ImmutableList<SymbolPair> idPairs,
      ...
      final ImmutableList<Integer> pvIndicesRemaining = getPvIndices(REMAINING); // for any remaining predicates
      ...
      for (final SymbolPair item : idPairs) {
        ...
        final ImmutableSet<Symbol> words1 = ImmutableSet.copyOf(Iterables.filter(examples.get(id1).predicates(), Predicates.in(predicatesWithEmbeddings)));
        ...
        if (maxSim > 0) {
          ...
          final ImmutableSet<Symbol> w1Remainder = maxPrimeWord1!=null? Sets.difference(w1Prime, ImmutableSet.of(maxPrimeWord1)).immutableCopy() : w1Prime;
          final ImmutableSet<Symbol> w2Remainder = maxPrimeWord2!=null? Sets.difference(w2Prime, ImmutableSet.of(maxPrimeWord2)).immutableCopy() : w2Prime;
          if(w1Remainder.size() > 0 && w2Remainder.size() > 0) {
            final ImmutableMap<Integer, Double> word1RemainingFeatures = getFeatureMappings(w1Remainder, pvIndicesRemaining);
            final ImmutableMap<Integer, Double> word2RemainingFeatures = getFeatureMappings(w2Remainder, pvIndicesRemaining);
            multiFeaturesBuilder.put(item, WeightedIndicesPair.from(word1RemainingFeatures, word2RemainingFeatures));
          }
    ...
    public MaxSimBiPredicateAll build() {
      ...
      if(indices.size() > 0) {
        ...
        return new MaxSimBiPredicateAll(featureName, features.build(), featureIndices, minIndex,
            maxIndex);
      } else {
        return new MaxSimBiPredicateAll(featureName, features.build(), featureIndices, -1, -1);
      }
    }
