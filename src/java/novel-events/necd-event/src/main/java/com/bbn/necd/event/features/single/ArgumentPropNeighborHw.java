 ...
 */
public final class ArgumentPropNeighborHw extends SingleFeature {
  private static final Logger log = LoggerFactory.getLogger(ArgumentPropNeighborHw.class);
  ...
  public static void insertFeatures(final String featureName,
    ...
    final ImmutableSet<SymbolPair> sourceNeighborPropLabelAndHw =
        PropositionTreeFeatures.toPropLabelAndHeadword(eg.sourceNeighbors());
    for (final SymbolPair pair : sourceNeighborPropLabelAndHw) {
      final String v =
          featureName + DELIMITER + PropositionTreeFeatures.SOURCE + DELIMITER + pair
              .getFirstMember().asString() + DELIMITER + pair.getSecondMember().asString();
      featuresCacheBuilder.put(item, v, 1.0);
    }

    final ImmutableSet<SymbolPair> targetNeighborPropLabelAndHw =
        PropositionTreeFeatures.toPropLabelAndHeadword(eg.targetNeighbors());
    for (final SymbolPair pair : targetNeighborPropLabelAndHw) {
      final String v =
