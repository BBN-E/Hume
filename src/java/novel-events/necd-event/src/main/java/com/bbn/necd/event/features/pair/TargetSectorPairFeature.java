...
public final class TargetSectorPairFeature extends PairFeature {
  private TargetSectorPairFeature(final String featureName,
  ...
  public static class Builder {
    ...
    public Builder extractFeatures(final ImmutableList<SymbolPair> idPairs,
      ...
      for (final SymbolPair item : idPairs) {
        ...
        final ImmutableSet<StringPair> values = getTargetSectorPairs(eg1, eg2);
    ...
    private Multiset<StringPair> getFeatureCount(final ImmutableList<SymbolPair> idPairs,
      ...
      for (final SymbolPair item : idPairs) {
        ...
        ret.addAll(getTargetSectorPairs(eg1, eg2));
      }

      return ret;
    }

    private ImmutableSet<StringPair> getTargetSectorPairs(final EventFeatures eg1, final EventFeatures eg2) {
      final ImmutableSet.Builder<StringPair> ret = ImmutableSet.builder();

      for (final Symbol s1 : eg1.targetSectors()) {
        for (final Symbol s2 : eg2.targetSectors()) {
    ...
    public TargetSectorPairFeature build() {
      ...
      if(indices.size() > 0) {
        ...
        return new TargetSectorPairFeature(featureName, features.build(), featureIndices, minIndex,
            maxIndex);
      } else {
        return new TargetSectorPairFeature(featureName, features.build(), featureIndices, -1, -1);
      }
