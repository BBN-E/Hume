 ...
 */
public final class ArgumentPropNeighborEntityType extends PairFeature {
      ...
      for (final SymbolPair idPair : idPairs) {
        for(final Symbol item : idPair.getMembers()) {
          ...
          if(!seenIds.contains(item)) {
            ...
            com.bbn.necd.event.features.single.ArgumentPropNeighborEntityType.insertFeatures(featureName, featuresCacheBuilder, eg, item);
