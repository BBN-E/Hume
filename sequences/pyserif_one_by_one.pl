...
my $jserif_event_jar;
BEGIN{
    $textopen_root = "/d4m/nlp/releases/text-open/R2020_05_07";
    # $hume_repo_root = "/d4m/nlp/releases/Hume/R2020_05_07_2";
    $hume_repo_root = Cwd::abs_path(__FILE__ . "/../..");
    $learnit_root = "/d4m/nlp/releases/learnit/R2020_05_01";
    $nlplingo_root = "/d4m/nlp/releases/nlplingo/R2020_05_07";
...
if (exists $stages{"serif"}) {
    ...
    my $run_fact_finder = get_param($params, "serif_run_fact_finder", "false");
                ...
                my $serif_jobid =
                    runjobs(
                        $split_serif_jobid, $serif_job_name,
                        {
                            ...
                            run_fact_finder                   => $run_fact_finder,
...
if ((exists $stages{"bert"}) && $use_bert) {
    ...
    my $list_sent_info_list_by_docid_for_s1_jobid;
    {
        ...
        for (my $n = 0; $n < $number_of_processes_bert_io; $n++) {
            ...
            my $producing_bert_tokens = runjobs(
                ...
                [ "env PYTHONPATH=$BERT_PYTHON_PATH $ANACONDA_ROOT/envs/$CONDA_ENV_NAME_FOR_BERT_CPU/bin/python $BERT_TOKENIZER_PATH --filelist $batch_file\.with_type --bert_vocabfile $BERT_VOCAB_FILE --outdir $token_map_dir_batch --maximum_allowed_bert_tokens_per_sentence $maximum_allowed_bert_tokens_per_sentence --do_lower_case True" ]
            );
    ...
    }
    {
        ...
        $generate_token_num_reverse_list_jobid = runjobs(
            ...
            [ "$PYTHON3 $BERT_GENERATE_FILE_LIST_BY_NUMBER_OF_TOKENS --input_sent_info_list $sent_info_list_by_docid_for_s1 --number_of_batches $number_of_batches_modified --batch_prefix $batch_prefix_for_sorted_sentences --output_path $output_sorted_sentences_dir --bert_job_list_path $bert_job_list_path" ]
        );
    }
...

    {
        ...
        for (my $n = 0; $n < $number_of_batches_modified; $n++) {
            ...
            my $generate_npz_jobid = runjobs(
                ...
                [ "$ANACONDA_ROOT/envs/$CONDA_ENV_NAME_FOR_BERT_CPU/bin/python $BERT_NPZ_EMBEDDING --batch_dir $batch_dir --token_map_list $token_map_list --outdir $batch_out_dir" ]
...
if (exists $stages{"learnit-decoder"}) {
    ...
    my $cserif_causal_json_stage_processing_dir;
    {
        ...
        my $learnit_event_and_event_arg_pattern_dir = "";
        if ($mode eq "CauseEx") {
            $learnit_event_and_event_arg_pattern_dir = get_param($params, "learnit_event_and_event_arg_pattern_dir") eq "DEFAULT"
                ? "$hume_repo_root/resource/learnit_patterns/unary_event_icms.legacy,$hume_repo_root/resource/learnit_patterns/unary_event_icms"
                : get_param("learnit_event_and_event_arg_pattern_dir");
        }
        else {
            $learnit_event_and_event_arg_pattern_dir = get_param($params, "learnit_event_and_event_arg_pattern_dir") eq "DEFAULT"
                ? "$hume_repo_root/resource/learnit_patterns/event_and_event_arg.legacy,$hume_repo_root/resource/learnit_patterns/event_and_event_arg"
                : get_param("learnit_event_and_event_arg_pattern_dir");
        }
        my $learnit_event_event_relation_pattern_dir = get_param($params, "learnit_event_event_relation_pattern_dir") eq "DEFAULT" ? "$hume_repo_root/resource/learnit_patterns/binary_event_event.legacy,$hume_repo_root/resource/learnit_patterns/binary_event_event" : get_param($params, "learnit_event_event_relation_pattern_dir");
        ...
        ($learnit_decoder_jobids, $output_learnit_seriflist_path) = $learnit_decoding_obj->LearnItDecoding(
            ...
            runjobs_template_hash => {
                stages_to_run                                    => "generic_event,unary_event_and_binary_event_argument_decoding,binary_event_event_decoding",
                max_number_of_tokens_per_sentence                => $max_number_of_tokens_per_sentence_global,
    ...
    #### End code block
    $LEARNIT_DECODER_SERIFXML = "$cserif_causal_json_stage_processing_dir/cserif_causal_json_serif.list";
}
dojobs();
...
if (exists $stages{"pyserif"}) {
    ...
    my $output_pyserif_main_doc_list;
    {
        ...
        if ($mode eq "CauseEx") {
            $internal_causal_factor_ontology = "$internal_ontology_dir/icms/event_ontology.yaml";
        ...
        my $pyserif_template = {
            ...
            cfOntologyFile                     => "$internal_ontology_dir/icms/event_ontology.yaml",
        ...
        my $updated_serif_list_path = $input_serifxml_list;
        (my $stage_processing_dir, my $mkdir_jobids) = Utils::make_output_dir("$processing_dir/$stage_name/$mini_stage_name", "$JOB_NAME/$stage_name/mkdir_stage_processing", []);
        my $wait_job = $mkdir_jobids;
        {
            $mini_stage_name = "nlplingo";
            (my $ministage_processing_dir, my $mkdir_jobs) = Utils::make_output_dir("$processing_dir/$stage_name/$mini_stage_name", "$JOB_NAME/$stage_name/$mini_stage_name/mkdir_stage_processing", $wait_job);
            my $pyserif_obj = PySerif->new();
            ($wait_job, $updated_serif_list_path) = $pyserif_obj->pyserif_pipeline(
                dependant_job_ids       => $mkdir_jobs,
                job_prefix              => "$JOB_NAME/$stage_name/$mini_stage_name",
                num_batches             => $NUM_OF_BATCHES_GLOBAL,
                input_doc_list          => $updated_serif_list_path,
                PYTHON                  => "$ANACONDA_ROOT/envs/$CONDA_ENV_NAME_FOR_DOC_RESOLVER/bin/python",
                PYTHONPATH              => "$nlplingo_root",
                pipeline_template_path  => "pyserif_one_by_one_nlplingo.par",
                pipeline_template_items => $pyserif_template,
                output_dir              => $ministage_processing_dir,
                schedule_processes      => 0,
            );
        }
        {
            $mini_stage_name = "ec";
            ...
            ($wait_job, $updated_serif_list_path) = $pyserif_obj->pyserif_pipeline(
                ...
                PYTHON                  => "$ANACONDA_ROOT/envs/$CONDA_ENV_NAME_FOR_DOC_RESOLVER/bin/python",
                PYTHONPATH              => "$nlplingo_root",
                pipeline_template_path  => "pyserif_one_by_one_ec.par",
                pipeline_template_items => $pyserif_template,
                output_dir              => $ministage_processing_dir,
                schedule_processes      => 0,
            );
        }
        {
            ...
            ($wait_job, $updated_serif_list_path) = $pyserif_obj->pyserif_pipeline(
                ...
                PYTHON                  => "$ANACONDA_ROOT/envs/$CONDA_ENV_NAME_FOR_DOC_RESOLVER/bin/python",
                PYTHONPATH              => "$nlplingo_root",
                pipeline_template_path  => "pyserif_one_by_one_pg.par",
                pipeline_template_items => $pyserif_template,
                output_dir              => $ministage_processing_dir,
                schedule_processes      => 0,
            );
        }
        {
            ...
            ($wait_job, $updated_serif_list_path) = $pyserif_obj->pyserif_pipeline(
                ...
                PYTHON                  => "$ANACONDA_ROOT/envs/$CONDA_ENV_NAME_FOR_DOC_RESOLVER/bin/python",
                PYTHONPATH              => "$nlplingo_root",
                pipeline_template_path  => "pyserif_one_by_one_ec_pass_2.par",
                pipeline_template_items => $pyserif_template,
                output_dir              => $ministage_processing_dir,
                schedule_processes      => 0,
            );
        }
        $GENERATED_PYSERIF_SERIFXML = $updated_serif_list_path;
    }
}
...
if (exists $stages{"serialization"}) {
    ...
    my $copy_serifxml_jobid =
        runjobs(
            ...
            [ "$PYTHON3 $COPY_FILES_SCRIPT $input_serifxml_list $serifxml_dir \"$input_metadata_file\"" ]
        );
    dojobs();
    ...
    if (is_run_mode()) {
        ...
        @document_types = grep {-d "$serifxml_dir/$_" && !/^\.{1,2}$/} readdir($dh);
    }

    foreach my $document_type (@document_types) {
        (my $serializer_output_dir, $mkdir_jobid) = Utils::make_output_dir("$GENERATED_SERIALIZATION_DIR/$document_type", "$JOB_NAME/serialize/mkdir_" . $document_type, []);
