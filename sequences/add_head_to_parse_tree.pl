...
my $job_prefix = "gigaword_eer_json_add_head";
my $number_of_batches = 1000;
my $output_dir = "$expt_dir/expts/$job_prefix";
my $input_serif_list = "/home/hqiu/ld100/Hume_pipeline_int/Hume/expts/gigaword_eer_json/serifxml.list";
...
for (my $batch = 0; $batch < $number_of_batches; $batch++) {
    ...
    my $learnit_decoder_jobid = runjobs4::runjobs(
        ...
        [ "env PYTHONPATH=$textopen_root/src/python $PYTHON3 $hume_root/src/python/misc/add_head_to_parse_tree.py --input_list $batch_file --output_dir $batch_serifxml_output_dir" ]
    );
