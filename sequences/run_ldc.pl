...
# /nfs/raid88/u10/users/jcai/expts/46692_ldc_11_21_2019/ckpt_train_ldc-E82_20191125_13_24_55";
...
if (exists $stages{"event-event-relations"}) {
    ...
    for (my $n = 0; $n < $NUM_JOBS; $n++) {
        ...
        my $nre_decoding_jobid_old = runjobs(
            [ $learnit_mappings_jobid ], "$JOB_NAME/$stage_name/NRE-OLD-$job_batch_num",
            {
                SGE_VIRTUAL_FREE => "25G",
                BATCH_QUEUE      => $LINUX_QUEUE,
                job_retries      => 1,
            ...
            [ "$NEURAL_RELATION_DECODER_SCRIPT_OLD", "learnit_minimal.par", "$mappings_output_file $nre_output_folder $learnit_root $deepinsight_root $nre_model_root $ANACONDA_ROOT py2-tf1 $JCAI_PT_CPU $ldc_model_root" ]
        );
        ...
        push(@nre_decoding_jobs, $nre_decoding_jobid_old);
    ...
    # Combine NRE decoding result.
    my $generate_nre_decoding_result_list_jobid = generate_file_list(\@nre_decoding_jobs, "$JOB_NAME/$stage_name/list_nre_decoding_result", "$stage_processing_dir/*/NRE/bag_predictions_learnit.json", "$stage_processing_dir/list_nre_decoding_result.list");
    my $combine_nre_decoding_result_jobid = runjobs(
        [ $generate_nre_decoding_result_list_jobid ], "$JOB_NAME/$stage_name/combine_nre_decoding_result",
        {
            BATCH_QUEUE => $LINUX_QUEUE,
        },
        [ "$PYTHON3 $LEARNIT_JSON_AGGREGATOR_SCRIPT $stage_processing_dir/list_nre_decoding_result.list $nre_relations_file_old.aggr" ]
    );

    my $nre_postprocessing_job_id =
        runjobs(
            [ $combine_nre_decoding_result_jobid ], "$JOB_NAME/$stage_name/nre_postprocessing",
            {
                BATCH_QUEUE => $LINUX_QUEUE,
            },
            [ "$PYTHON3 $NRE_POSTPROCESSING $nre_relations_file_old.aggr $nre_relations_file_old.filtered" ]
        );

    my $nre_rescale_freq_job_id =
        runjobs(
            [ $nre_postprocessing_job_id ], "$JOB_NAME/$stage_name/nre_rescale_freq",
            {
                BATCH_QUEUE => $LINUX_QUEUE,
            },
            [ "$PYTHON3 $NRE_RESCALE_FREQ $nre_relations_file_old.filtered $nre_relations_file_old.rescaled" ]
        );

    my $nre_strip_low_conf_job_id =
        runjobs(
            [ $nre_rescale_freq_job_id ], "$JOB_NAME/$stage_name/nre_strip_low_conf",
            {
                BATCH_QUEUE => $LINUX_QUEUE,
            },
            [ "$PYTHON3 $NRE_STRIP_LOW_CONF $nre_relations_file_old.rescaled $nre_relations_file_old 0.4" ]
        );
    ...
    my $nre_strip_low_conf_job_id_ldc =
        runjobs(
            ...
            [ "$PYTHON3 $NRE_STRIP_LOW_CONF $nre_relations_file_ldc.rescaled $nre_relations_file_ldc 0.6" ]
        );

    my $nre_decode_unify =         runjobs(
            [ $nre_strip_low_conf_job_id, $nre_strip_low_conf_job_id_ldc ], "$JOB_NAME/$stage_name/nre_decode_unify",
            {
                BATCH_QUEUE => $LINUX_QUEUE,
            },
